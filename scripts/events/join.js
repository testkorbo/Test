...
.catch(error => {
    ...
    try {
      if (!fs.existsSync(__dirname + `/Alvi/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/Alvi/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        ...
        registerFont(__dirname + `/Alvi/font/Semi.ttf`, {
          family: "Semi"
        });
